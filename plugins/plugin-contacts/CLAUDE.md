...
- **View authority.** The view and provider require `ADMIN`. `list-contacts` requests the native signed 32-bit maximum and returns data only when the bridge result proves it stayed below that boundary; reaching `2,147,483,647` rows is a typed incomplete-read error, never a planner-facing prefix. `create-contact` and `import-vcard` are registered as human-only. The view intentionally omits the `agent-surface` grant, so generic `agent-fill`/`agent-click` cannot bypass those classifications.
