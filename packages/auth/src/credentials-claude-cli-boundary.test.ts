...
describe("Claude Code credential boundary", () => {
  ...
  it("preserves a future camelCase expiry", async () => {
    ...
    expect(info).toHaveBeenCalledWith(
      expect.stringContaining("Imported OAuth token from Claude Code"),
    );
    expect(info).toHaveBeenCalledWith(
      expect.stringContaining("detected via Claude Code CLI"),
    );
  });
  ...
  it("refreshes an expired snake_case credential", async () => {
    const expiresAt = Date.now() - 60_000;
    useClaudeCredentialFixture({
      access_token: "access",
    ...
    vi.mocked(refreshAnthropicToken).mockResolvedValue({
      access: "fresh-access",
    ...
    expect(refreshAnthropicToken).toHaveBeenCalledOnce();
    expect(refreshAnthropicToken).toHaveBeenCalledWith("refresh");
  });
});
